...
public class certainTestResultsLayout extends Layout{

	private final Test test;
	
	public certainTestResultsLayout(VBox vBox, Test test) {
		super(vBox);
		this.test = test;
	}
		...
		try {
			...
			if(response.getType() == ResponseType.Unsuccessful) {
				...
				for(int i = testsResults.size() - 1; i >= 0; i--) {
					...
					if(!testResult.getTest().getName().equals(test.getName())) continue;
